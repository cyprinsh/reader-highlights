...
def parse_mrexpt(file_bytes, filename):
    ...
    # 괄호와 저자명 제거하고 순수 책 제목만 추출하여 필터링에 활용
    ...
    for line in lines:
        ...
        # [필터 1] 양수/음수 일체 숫자 제거 (-2004318072 같은 시스템 ID 코드 제거)
        if re.match(r'^-?\d+$', text): continue
        
        # [필터 2] 북리더 내부 설정값 제거 (indent:false, trim:false, true 등)
        ...
        # [필터 4] 본문 내 불필요하게 반복되는 책 제목 제거
        if text.lower() == book_title.lower() or text.lower() == raw_title.lower(): continue
...
def generate_html(book_data):
    ...
    <head>
        ...
        <style>
            ...
            .footer {{ text-align: center; font-size: 0.8em; color: #bdc3c7; margin-top: 50px; }}
        </style>
    </head>
    <body>
        <div class="container">
    ...
    html_content += """
            <div class="footer">Generated by Moon+ Reader 웹 대시보드</div>
        </div>
    </body>
    </html>
    """
...
with st.expander("📂 여기에 여러 파일 드래그 앤 드롭하기", expanded=not bool(st.session_state.books_db)):
    uploaded_files = st.file_uploader(
        ...
        type=["mrexpt"], 
        accept_multiple_files=True # 여러 파일 드래그 앤 드롭 허용
    )
...
if st.session_state.books_db:
    ...
    # [기능] Streamlit 내부에 완벽한 HTML 결과물 직접 임베딩하여 보여주기
    components.html(html_string, height=600, scrolling=True)

else:
    st.info("💡 위 상자를 클릭하여 북리더 백업 파일(.mrexpt)들을 마우스로 끌어다 놓아주세요.")
